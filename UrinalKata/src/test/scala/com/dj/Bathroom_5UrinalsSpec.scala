...
class Bathroom_5UrinalsSpec extends FlatSpec with Matchers {
  "Bathroom" should "return position 1 when 5 urinals exist and 2, 3, 5 are status" in {
  ...
  "Bathroom" should "return position 1 when 5 urinals exist and 2, 4, 5 are status" in {
    val bathroom: Bathroom = new Bathroom(5)
    bathroom.getUrinalByPosition(2).get.status = Occupied
